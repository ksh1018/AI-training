/**
 * @file rex_display.cc
 * @author Seunghun Kim (you@domain.com)
 * @brief Rex display interface source
 * @version 0.1
 * @date 2020-06-30
 * 
...
static void modeset_draw(uint8_t *ptr) {
    ...
    struct drm_dev *iter;
    for (iter = modeset_list; iter; iter = iter->next) {
        memcpy((uint8_t *)&iter->map[0], &ptr[0], DISP_W * DISP_H * DISP_CH);  // SEGV
    }
}
...
static void InitRgaInfo(rga_info_t *src, rga_info_t *dst, void *dstBuffer) {
...
void drawFrame(uint8_t *frame_buf) {
    ...
    static int rgaInit = 0;
    static rga_info_t resize_src;
    static rga_info_t resize_dst;
    static void *dst_v;

    if (rgaInit == 0) {
        c_RkRgaInit();
        dst_v = malloc(DISP_W * DISP_H * DISP_CH);
        InitRgaInfo(&resize_src, &resize_dst, dst_v);

        puts("RkRga Init");
        rgaInit = 1;
    }

    int rawsize = SRC_HEIGHT * SRC_WIDTH * SRC_BPP;
    // RGA Resizing
    resize_src.virAddr = frame_buf;
    
    int ret            = c_RkRgaBlit(&resize_src, &resize_dst, NULL);
    modeset_draw((uint8_t *)dst_v);

    auto t2       = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1).count();
    //std::cout << "Display elapsed: " << duration << "ms" << std::endl;
}